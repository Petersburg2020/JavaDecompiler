...
import nx.peter.lib.jd_core.service.converter.classfiletojavasyntax.model.javasyntax.statement.ClassFileBreakContinueStatement;
import nx.peter.lib.jd_core.service.converter.classfiletojavasyntax.model.javasyntax.statement.ClassFileForEachStatement;
import nx.peter.lib.jd_core.service.converter.classfiletojavasyntax.model.javasyntax.statement.ClassFileForStatement;
...
import nx.peter.lib.jd_core.service.converter.classfiletojavasyntax.visitor.*;
...
import static nx.peter.lib.jd_core.model.javasyntax.statement.ContinueStatement.CONTINUE;
import static nx.peter.lib.jd_core.model.javasyntax.type.ObjectType.TYPE_ITERABLE;
...
public class LoopStatementMaker {
    ...
    protected static Statement makeLoop(int majorVersion, Map<String, BaseType> typeBounds, LocalVariableMaker localVariableMaker, BasicBlock loopBasicBlock, Statements statements, Expression condition, Statements subStatements) {
                ...
                if (lineNumber > 0) {
                    ...
                    if (subStatement.isExpressionStatement()) {
                        ...
                        if (subExpression.getLineNumber() == lineNumber) {
                            return newClassFileForStatement(localVariableMaker, loopBasicBlock.getFromOffset(), loopBasicBlock.getToOffset(), init, condition, subExpression, null);
                        } else if (init != null) {
                            return newClassFileForStatement(localVariableMaker, loopBasicBlock.getFromOffset(), loopBasicBlock.getToOffset(), init, condition, null, subStatement);
                        }
                    } else if (init != null) {
                        return newClassFileForStatement(localVariableMaker, loopBasicBlock.getFromOffset(), loopBasicBlock.getToOffset(), init, condition, null, subStatement);
                    }
        ...
        if (lineNumber > 0) {
            switch (statements.size()) {
                ...
                case 1:
                    ...
                    if ((expression.getLineNumber() != lineNumber) || !expression.isBinaryOperatorExpression() || (expression.getRightExpression().getLineNumber() == 0)) {
                        break;
                    }
                ...
                default:
                    ...
                    if (init.size() > 0) {
                        if (init.size() > 1) Collections.reverse(init);
                        return init;
                    }
                    break;
            }
    ...
    protected static Statement createForStatementWithoutLineNumber(LocalVariableMaker localVariableMaker, BasicBlock basicBlock, Statements statements, Expression condition, Statements subStatements) {
        ...
        if (!statements.isEmpty()) {
            ...
            if (init.getLeftExpression().isLocalVariableReferenceExpression()) {
                ...
                if (expression.isLocalVariableReferenceExpression() && (((ClassFileLocalVariableReferenceExpression) expression).getLocalVariable() == localVariable)) {
                    ...
                    return newClassFileForStatement(localVariableMaker, basicBlock.getFromOffset(), basicBlock.getToOffset(), init, condition, update, subStatements);
                }
            }
        }
    ...
    protected static Statement makeForEachList(Map<String, BaseType> typeBounds, LocalVariableMaker localVariableMaker, Statements statements, Expression condition, Statements subStatements) {
        ...
        if (!mie.getName().equals("hasNext") || !mie.getInternalTypeName().equals("java/util/Iterator") || !mie.getExpression().isLocalVariableReferenceExpression()) {
            return null;
        }
        ...
        if (!mie.getName().equals("next") || !mie.getInternalTypeName().equals("java/util/Iterator") || !mie.getExpression().isLocalVariableReferenceExpression()) {
            return null;
        }
    ...
    protected static ClassFileForStatement newClassFileForStatement(LocalVariableMaker localVariableMaker, int fromOffset, int toOffset, BaseExpression init, Expression condition, BaseExpression update, BaseStatement statements) {
        if (init != null) {
