...
import nx.peter.lib.jd_core.service.converter.classfiletojavasyntax.model.javasyntax.expression.*;
...
import static nx.peter.lib.jd_core.model.javasyntax.declaration.Declaration.FLAG_STATIC;
...
public class Java5TypeParametersToTypeArgumentsBinder extends AbstractTypeParametersToTypeArgumentsBinder implements ExpressionVisitor {
    ...
    public ClassFileMethodInvocationExpression newMethodInvocationExpression(int lineNumber, Expression expression, ObjectType objectType, String name, String descriptor, TypeMaker.MethodTypes methodTypes, BaseExpression parameters) {
        return new ClassFileMethodInvocationExpression(lineNumber, methodTypes.typeParameters, methodTypes.returnedType, expression, objectType.getInternalName(), name, descriptor, clone(methodTypes.parameterTypes), parameters);
    }
    ...
    protected boolean populateBindings(Map<String, TypeArgument> bindings, Expression expression, BaseTypeParameter typeParameters, BaseTypeArgument typeArguments, BaseTypeParameter methodTypeParameters, Type returnType, Type returnExpressionType, BaseType parameterTypes, BaseExpression parameters) {
        ...
        if (parameterTypes != null) {
            if (parameterTypes.isList() && parameters.isList()) {
                ...
                for (Expression parameter : parameters) {
                    populateBindingsWithTypeArgument(bindings, typeBounds, parameterTypesIterator.next(), parameter);
                }
            } else {
