...
import nx.peter.lib.jd_core.model.classfile.attribute.AttributeBootstrapMethods;
import nx.peter.lib.jd_core.model.classfile.attribute.AttributeCode;
import nx.peter.lib.jd_core.model.classfile.attribute.BootstrapMethod;
import nx.peter.lib.jd_core.model.classfile.constant.*;
...
import nx.peter.lib.jd_core.service.converter.classfiletojavasyntax.model.javasyntax.declaration.*;
import nx.peter.lib.jd_core.service.converter.classfiletojavasyntax.model.javasyntax.expression.ClassFileCmpExpression;
...
import nx.peter.lib.jd_core.service.converter.classfiletojavasyntax.model.javasyntax.statement.ClassFileMonitorEnterStatement;
import nx.peter.lib.jd_core.service.converter.classfiletojavasyntax.model.javasyntax.statement.ClassFileMonitorExitStatement;
import nx.peter.lib.jd_core.service.converter.classfiletojavasyntax.model.localvariable.AbstractLocalVariable;
import nx.peter.lib.jd_core.service.converter.classfiletojavasyntax.visitor.EraseTypeArgumentVisitor;
import nx.peter.lib.jd_core.service.converter.classfiletojavasyntax.visitor.RenameLocalVariablesVisitor;
import nx.peter.lib.jd_core.service.converter.classfiletojavasyntax.visitor.SearchFirstLineNumberVisitor;
import nx.peter.lib.jd_core.util.DefaultList;
import nx.peter.lib.jd_core.util.DefaultStack;

import java.util.*;

import static nx.peter.lib.jd_core.model.javasyntax.declaration.Declaration.*;
import static nx.peter.lib.jd_core.model.javasyntax.statement.ReturnStatement.RETURN;
import static nx.peter.lib.jd_core.model.javasyntax.type.ObjectType.*;
import static nx.peter.lib.jd_core.model.javasyntax.type.PrimitiveType.TYPE_BOOLEAN;
import static nx.peter.lib.jd_core.model.javasyntax.type.PrimitiveType.TYPE_BYTE;
import static nx.peter.lib.jd_core.model.javasyntax.type.PrimitiveType.TYPE_DOUBLE;
import static nx.peter.lib.jd_core.model.javasyntax.type.PrimitiveType.TYPE_FLOAT;
import static nx.peter.lib.jd_core.model.javasyntax.type.PrimitiveType.TYPE_LONG;
import static nx.peter.lib.jd_core.model.javasyntax.type.PrimitiveType.TYPE_SHORT;
import static nx.peter.lib.jd_core.model.javasyntax.type.PrimitiveType.*;
...
public class ByteCodeParser {
                    ...
                    if (opcode == 184) { // INVOKESTATIC
                        ...
                        if (TYPE_VOID.equals(methodTypes.returnedType)) {
                            if ((opcode == 183) && // INVOKESPECIAL
                                ...
                                expression1 = typeParametersToTypeArgumentsBinder.newMethodInvocationExpression(lineNumber, getMethodInstanceReference(expression1, ot, name, descriptor), ot, name, descriptor, methodTypes, parameters);
                                typeParametersToTypeArgumentsBinder.bindParameterTypesWithArgumentTypes(TYPE_OBJECT, expression1);
                                statements.add(new ExpressionStatement(expression1));
                            }
                        } else {
                            ...
                            stack.push(typeParametersToTypeArgumentsBinder.newMethodInvocationExpression(lineNumber, getMethodInstanceReference(expression1, ot, name, descriptor), ot, name, descriptor, methodTypes, parameters));
                        }
                    }
                    break;
        ...
        if (typeName.equals(internalTypeName)) {
            for (ClassFileConstructorOrMethodDeclaration methodDeclaration : bodyDeclaration.getMethodDeclarations()) {
                if (((methodDeclaration.getFlags() & (FLAG_SYNTHETIC | FLAG_PRIVATE)) == (FLAG_SYNTHETIC | FLAG_PRIVATE)) && methodDeclaration.getMethod().getName().equals(name1) && methodDeclaration.getMethod().getDescriptor().equals(descriptor1)) {
                    ...
                    stack.push(new LambdaIdentifiersExpression(lineNumber, indyMethodTypes.returnedType, indyMethodTypes.returnedType, prepareLambdaParameterNames(cfmd.getFormalParameters(), parameterCount), prepareLambdaStatements(cfmd.getFormalParameters(), indyParameters, cfmd.getStatements())));
                    return;
                }
            }
        }
    ...
    private static boolean isPositiveOne(Expression expression) {
        if (expression.isIntegerConstantExpression() && expression.getIntegerValue() == 1) return true;
        if (expression.isLongConstantExpression() && expression.getLongValue() == 1L) return true;
        if (expression.isFloatConstantExpression() && expression.getFloatValue() == 1.0F) return true;
    ...
    private void parseIF(DefaultStack<Expression> stack, int lineNumber, BasicBlock basicBlock, String operator1, String operator2, int priority) {
        ...
        if (expression.getClass() == ClassFileCmpExpression.class) {
            ...
            switch (pt.getJavaPrimitiveFlags()) {
                case FLAG_BOOLEAN:
                    if (basicBlock.mustInverseCondition() ^ "==".equals(operator1)) stack.push(expression);
                    else stack.push(new PreOperatorExpression(lineNumber, "!", expression));
                    break;
                case FLAG_FLOAT:
    ...
    private void parseXRETURN(Statements statements, DefaultStack<Expression> stack, int lineNumber) {
        ...
        if (!statements.isEmpty() && valueRef.isLocalVariableReferenceExpression()) {
            ...
            if (lastStatement.isExpressionStatement()) {
                ...
                if ((lineNumber <= expression.getLineNumber()) && expression.isBinaryOperatorExpression() && expression.getOperator().equals("=") && expression.getLeftExpression().isLocalVariableReferenceExpression()) {
    ...
    public static boolean isAssertCondition(String internalTypeName, BasicBlock basicBlock) {
        ...
        if (offset + 3 > toOffset) return false;
        ...
        if (!"$assertionsDisabled".equals(name)) return false;
